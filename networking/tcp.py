...
class MessageHandler:
    ...
    async def on_received(self, session, buffer, timestamp):
        pass
...
class Session:
  ...
  async def send(self, buffer):
    """
    Sends a byte buffer using the following format.
    ...
    """
    try:
      total_size = 4 + len(buffer)  # size prefix + payload
      message_bytes = (
  ...
  async def _run(self):
    while True:
      ...
      try:
        ...
        print(f"Size: {total_size:08x}")
        payload_size = total_size - 4
        if payload_size > 0:
          payload = await self._reader.readexactly(payload_size)
          try:
            await self._message_handler.on_received(session = self, buffer = payload, timestamp = timestamp)
          except Exception as e:
